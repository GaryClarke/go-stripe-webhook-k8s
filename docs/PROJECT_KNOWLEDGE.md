...
Work is done incrementally in **numbered branches** (git + `docs/branches/NN-*.md`). **§5 is the canonical sequence**—older ad-hoc “step 1–9” lists elsewhere should match this table.
...
|--------|-----|----------------|
| **01** | [docs/branches/01-foundation.md](branches/01-foundation.md) | Go module, project layout, tooling |
| **02** | [docs/branches/02-api-gateway.md](branches/02-api-gateway.md) | API Gateway HTTP API, `GET /healthz` → Lambda |
| **03** | [docs/branches/03-s3-backend-ci.md](branches/03-s3-backend-ci.md) | Terraform remote state (S3 + DynamoDB lock) |

There is **no `04` branch doc**; numbering continues at **05** for Phase 1 app work (see [03 next branch](branches/03-s3-backend-ci.md)).

### Phase 1: Application (local queue, no SQS in the critical path yet)

| Branch | Doc | What it covers |
|--------|-----|----------------|
| **05** | [docs/branches/05-domain-types.md](branches/05-domain-types.md) | `StripeEvent`, `Job`, `testdata` fixture |
| **06** | [docs/branches/06-config.md](branches/06-config.md) | Env config, godotenv, `QUEUE_BACKEND` validation |
| **07** | [docs/branches/07-queue-abstraction.md](branches/07-queue-abstraction.md) | `Enqueuer` / `Consumer`, in-memory `MemoryQueue` |
| **08** | [docs/branches/08-queue-from-config.md](branches/08-queue-from-config.md) | `NewFromConfig`, `memory` vs `sqs` stub |
| **09** | [docs/branches/09-parse-stripe-event.md](branches/09-parse-stripe-event.md) | `ParseStripeEvent`, tests against `testdata` fixture |

### Proposed branches (Phase 1 remainder)

Rough scope for upcoming work—**adjust as you go**; add `docs/branches/NN-*.md` when each branch lands. Git branch names can mirror (e.g. `10-job-from-event`).
...
See **[docs/branches/README.md](branches/README.md)** for a short ordered list of all branch markdown files.
