...
- **Commands / chat shorthands** (**aac**, **ms**, **mmp**, **start work**, etc.) are defined **only in this file** (Commits and branches § and **ROSA lab session** below). Do not copy their definitions into [PLAN.md](PLAN.md), [docs/branches/README.md](docs/branches/README.md), or elsewhere - link here instead.
...
- **start work** = begin a **ROSA lab session** ([**docs/rosa/runbook.md**](docs/rosa/runbook.md)). When the user says **`start work`** (or **start work on rosa**):
  ...
  3. **If exit 1** or the script prints **`>>> YOU:`** lines: quote those prompts verbatim and tell the user to run the listed commands **themselves**, then say **`start work`** again (or **`make lab-status`**). Typical **user-only** steps (interactive or secret):
     ...
     - **`oc login https://api.gc-rosa-lab.bd91.p1.openshiftapps.com:6443 -u garyc -p '...'`** — htpasswd password; never committed or pasted to the agent.
  ...
  4. **Do not** re-run full Phase 1 deploy (**`rosa create cluster`**, **`oc apply`**, IDP setup) on **`start work`** unless **`lab-status`** shows cluster/app missing.
  5. **Later (CI / Terraform):** **`start work`** stays useful for auth + health; deploy moves to **`push main`**. **`lab on/off`** scripts land when hibernate or Terraform **`lab_enabled`** exists.
