...
- **start work** = begin a **ROSA lab session** ([**docs/rosa/runbook.md**](docs/rosa/runbook.md)). When the user says **`start work`** (or **start work on rosa**):
  ...
  5. **Lab lifecycle:** end of day **`make lab-off`**; start of day **`make lab-on`** then checklist (GitHub **`ROSA_API_URL`**, Stripe URL, fresh-cluster IDP if needed). Deploy on **`push main`** is automatic when cluster is up (**`deploy-rosa.yaml`**).
